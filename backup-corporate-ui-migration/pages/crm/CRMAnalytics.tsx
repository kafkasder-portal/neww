              ...
              <CorporateCardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={analyticsData.campaignPerformance}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="raised" fill={colors.success[500]} name="Toplanan" />
            ...
            <CorporateCard>
              ...
              <CorporateCardContent className="space-y-4">
                {analyticsData.communicationActivity.map((activity, _index) => (
                  <div key={_index} className="flex items-center justify-between p-4 bg-corporate-neutral-50 rounded-lg">
                    <div className="flex items-center space-x-3">
                      <div className="bg-corporate-primary-100 p-2 rounded-full">
                        {activity.type === 'email' && <Mail className="w-5 h-5 text-corporate-primary-600" />}
                        {activity.type === 'phone' && <Phone className="w-5 h-5 text-corporate-primary-600" />}
                        {activity.type === 'meeting' && <Calendar className="w-5 h-5 text-corporate-primary-600" />}
                        {activity.type === 'message' && <MessageSquare className="w-5 h-5 text-corporate-primary-600" />}
                      </div>
                      <div>
                        <p className="font-medium text-corporate-text-primary">{activity.label}</p>
                        <p className="text-sm text-corporate-text-secondary">{activity.description}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-bold text-corporate-text-primary">{activity.count}</p>
                      <p className="text-sm text-corporate-text-secondary">bu dönem</p>
                    </div>
                  </div>
                ))}
              </CorporateCardContent>

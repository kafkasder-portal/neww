...
export function RealtimeTaskUpdates({ 
  ...
  if (loading) {
    return (
      <CorporateCard className={`p-6 ${className}`}>
        ...
        {tasks.length === 0 ? (
          ...
          <div className="space-y-3">
            {tasks.slice(0, 5).map(task => (
              <div
                key={task.id}
                className="p-4 border rounded-lg hover:corporate-table-header cursor-pointer transition-colors group"
        ...
        <CorporateCard className="p-6">
          ...
          <div className="space-y-3">
            {recentComments.map(comment => (
              <div key={comment.id} className="p-3 corporate-table-header rounded-lg">

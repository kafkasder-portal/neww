...
export default function DonationNumbers() {
  ...
  const columns: Column<DonationNumber>[] = [
    ...
    { key: 'expiryDate', header: 'Son Tarih' },
    {
      ...
      header: 'Durum',
      render: (_, row: DonationNumber) => (
        <span className={`px-2 py-1 rounded text-xs ${row.status === 'aktif' ? 'bg-green-100 text-green-800 border-green-200' :
            row.status === 'tamamlandı' ? 'bg-blue-100 text-blue-800 border-blue-200' :
              row.status === 'süresi doldu' ? 'bg-red-100 text-red-800 border-red-200' :
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 bg-card rounded-lg border rounded-lg shadow">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          />
          <button
            onClick={() => exportToCsv('bagis-numaralari.csv', filteredNumbers)}

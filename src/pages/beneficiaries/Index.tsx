...
import { getNextSequentialId, useBeneficiaries } from '@hooks/useBeneficiaries'
...
export default function Beneficiaries() {
    ...
    const handleCompleteTemp = (row: BeneficiaryRow) => {
        ...
        setEditingId(typeof row.id === 'string' || typeof row.id === 'number' ? Number(row.id) : null)
        setOpen(true)
    }

    // Demo kayıt veri setleri
    ...
    const columns: Column<BeneficiaryRow>[] = [
        { key: 'id', header: 'ID', render: (_, row, index) => <Link className="text-blue-600 underline" to={`/aid/beneficiaries/${String(row.id || '')}`}>{(index ?? 0) + 1}</Link> },
        ...
        { key: 'district', header: 'İlçe' },
        {
            ...
            header: 'Durum',
            render: (_value, row) => (
                <span className={`px-2 py-1 rounded text-xs font-medium ${row.status === 'suspended'
